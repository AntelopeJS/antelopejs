...
export namespace internal {
  export const sendEmailProxy =
    InterfaceFunction<(params: EmailParams) => Promise<EmailResult>>();
  export const getStatusProxy =
    InterfaceFunction<(messageId: string) => Promise<string>>();
}
...
### How the Core Tells the Two Kinds Apart

Both an interface package and a runtime module carry an `antelopeJs` key in
their `package.json`, so the key alone says nothing. The discriminator is
`antelopeJs.implements`:

| `antelopeJs.implements`       | The package is                                | Depending on it means                               |
| ----------------------------- | --------------------------------------------- | --------------------------------------------------- |
| absent, or `[]`               | an interface package                          | the contract must be provided by some loaded module |
| lists other packages          | a runtime module                              | a plain package dependency                          |
| lists only the package itself | a runtime module that ships its own interface | the contract must be provided                       |

This matters because the core refuses to start when a module depends on an
interface package nobody implements. Without the rule, a module that depends
on another runtime module — a playground depending on the product it
exercises, say — fails startup with
`Unresolved interface dependencies: <module> requires <runtime package>`, and
the only way out is to make the runtime package declare itself in its own
`implements`. Declare `implements` for what a module actually implements; a
package never has to name itself to be depended upon.

A package loaded as a module of the project is never treated as an interface
package, whatever its manifest says.
...
export namespace internal {
  export const getProxy =
    InterfaceFunction<(key: string) => Promise<StorageEntry | null>>();
  export const setProxy =
    InterfaceFunction<(key: string, value: string) => Promise<StorageEntry>>();
}
