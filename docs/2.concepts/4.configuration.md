...
A module skipped because its provider failed still owes the interfaces it declared. Its peers do not read config variables from it — they call it — so the async proxies of those interfaces reject instead of waiting, naming the module that never constructed and the interface. A boot that ends in failures reports them and leaves with a non-zero exit code, even when a module bound a socket before its peer threw.
