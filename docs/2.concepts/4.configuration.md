...
References of the form `${@<module>.<VAR_NAME>}` belong to the reserved [config variables](#module-config-variables) namespace: they are left untouched by this pass and resolved later, once their provider module constructed.
...
It then returns the values from its `construct` callback:

```typescript [index.ts]
import type { ConfigVars } from "@antelopejs/interface-core/config";

export async function construct(config: unknown): Promise<ConfigVars> {
  const port = await bindPort(config);
  return { API_PORT: port };
}
```

A module that publishes nothing keeps returning `undefined`, exactly as before.
...
A reference that makes up the whole string keeps the published type (a number stays a number); a reference embedded in a longer string is interpolated. Substitution reaches any depth of the configuration, arrays included.

### Resolution order

The core reads every declaration before constructing a single module, so the provider/consumer graph is known upfront:

1. Modules that wait on no variable construct first, concurrently.
2. Each provider's published values are pushed into the config of its consumers.
3. Those consumers construct, and so on until every module constructed.

Modules that share no variable always construct concurrently. Values are resolved **once** and frozen: a value that changes after startup is not propagated.
...
| Situation                                             | When                    |
| ----------------------------------------------------- | ----------------------- |
| A reference to a module that is not loaded             | Before the first construct |
| A reference to a variable the provider does not declare | Before the first construct |
| A cycle in the graph (`api -> dms -> api`)             | Before the first construct |
| A declared variable the `construct` did not return     | After the provider constructs |
| A provider whose `construct` failed                    | Its consumers are reported as failed and are never constructed with an unresolved config |
...
- **Config variables** (`${@api.API_PORT}`) reference a value another module publishes at construct time, so a runtime value lives in one place instead of being duplicated across module configs.
