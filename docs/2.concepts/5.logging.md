...
Module tracking controls which modules produce visible log output. When `enabled` is `true`, each log entry is tagged with its originating module and the tag is shown in the output as `(module-name)`.

A module listed in `excludes` is always hidden. When `includes` is non-empty, only the modules it lists are shown. Both fields can be set together, in which case a module must be listed in `includes` and absent from `excludes` to produce output.

Logs emitted by the framework itself rather than by one of your modules are attributed to the module name `core`. Filter them like any other module: add `"core"` to `excludes` to silence framework diagnostics, or include it alongside your own modules to keep them visible.
...
```typescript [antelope.config.ts]
{
  logging: {
    moduleTracking: {
      enabled: true,
      includes: ["my-module", "core"]
    }
  }
}
```

Setting `includes` without listing `core` hides framework diagnostics such as missing interface providers and shutdown failures.
...
The `dateFormat` field accepts a date format string (default: `"yyyy-MM-dd HH:mm:ss"`). Supported tokens: `yyyy`, `MM`, `dd`, `HH`, `mm`, `ss` (all values are formatted in the local timezone).
