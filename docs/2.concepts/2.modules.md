...
A module may also export an optional `provide` callback, which runs before any module constructs and returns the [config variables](/docs/concepts/configuration#module-config-variables) it publishes — the values other modules reference from their own configuration with `${@<module>.<VAR_NAME>}`. Only providers implement it, and it is the only place config variables are produced: a value returned from `construct` is ignored.
...
No module has constructed while `provide` runs, so it cannot await another module's interface.
