...
`construct` may also return the [config variables](/docs/concepts/configuration#module-config-variables) the module publishes — the values other modules reference from their own configuration with `${@<module>.<VAR_NAME>}`. A module that publishes nothing returns nothing.
