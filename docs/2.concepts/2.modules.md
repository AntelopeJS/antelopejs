...
## File Ownership on Reload

Everything a module file registers when it is evaluated — routes, pages, controllers, event handlers — belongs to the module that owns the file, and is released when that module generation is destroyed. Ownership follows the package a file ships in, not the module that happened to require it first.

A module that imports a file from another module's runtime package therefore does not take ownership of it: the core evaluates the file under the owning module's context, so its registrations survive the importing module's reload and come back when the owning module itself reloads. The core logs a warning for such an import, because it ties the consumer to symbols whose identity changes whenever the owner reloads.

::warning
Depend on interface packages, not on another module's runtime package. An interface package is shared by every consumer and survives their reloads; a runtime package belongs to a single module and its lifecycle.
::
