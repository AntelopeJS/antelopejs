...
`ajs project start` dispatches directly to the production runtime without
loading the developer CLI, scaffolding commands, or performing the CLI update
check against the npm registry. Use this command in production containers and
process managers after `ajs project build`.
