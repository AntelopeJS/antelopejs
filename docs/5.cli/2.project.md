...
For production containers and process managers, use the dedicated launcher:

```bash
ajs-start --project /srv/app --env production
```

`ajs-start` accepts project, environment, concurrency, and comma-separated
verbose channel options. It launches the build artifact directly without
loading the developer CLI, scaffolding commands, Commander, or performing the
CLI update check against the npm registry. Keep `ajs project start` when CLI
help and presentation are useful; both commands use the same runtime path and
build artifact.
