...
export async function moduleImportInstallCommand(interfaceArg: string | undefined, options: InstallOptions) {
  ...
  let allInterfaces = [...imports, ...optionalImports].map((interface_) => {
  ...
  if (interfaceArg) {
    const [filterName, filterVersion] = interfaceArg.split('@');
    allInterfaces = allInterfaces.filter((iface) => {
      if (filterVersion) {
        return iface.name === filterName && iface.version === filterVersion;
      }
      return iface.name === filterName;
    });

    if (allInterfaces.length === 0) {
      warning(chalk.yellow`Interface '${interfaceArg}' not found in package.json imports`);
      info(`Use 'ajs module imports list' to see available imports.`);
      return;
    }
  }
  ...
  if (allInterfaces.length === 0) {
    info(chalk.blue`No interfaces defined in package.json`);
    info(`Add interfaces using 'ajs module imports add' first.`);
    return;
  }
...
export default function () {
  return new Command('install')
    ...
    .argument('[interface]', 'Specific interface to install (format: interface@version)')
    .action(moduleImportInstallCommand);
}
