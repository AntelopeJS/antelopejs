  ...
  return new Command("install")
    ...
    .action(async (options: InstallOptions) => {
      ...
      if (!moduleManifest.antelopeJs) {
        warning(
          chalk.yellow`No AntelopeJS configuration found in package.json`,
        );
      ...
      const allInterfaces = [...imports, ...optionalImports].map(
        (interface_) => {
          const name = mapModuleImport(interface_);
          const [interfaceName, version] = name.split("@");
          const git =
            typeof interface_ === "object" ? interface_.git : undefined;
          const skipInstall =
            typeof interface_ === "object" ? interface_.skipInstall : undefined;
      ...
      if (missingInterfaces.length === 0) {
        ...
        info(
          `Found ${allInterfaces.length} interface(s) in package.json, all are present in .antelope/interfaces.d`,
        );
      ...
      info(
        chalk.blue`Found ${missingInterfaces.length} missing interface(s) out of ${allInterfaces.length} required:`,
      );
      missingInterfaces.forEach((interface_) => {
        info(
          `  ${chalk.yellow("•")} ${chalk.bold(`${interface_.name}@${interface_.version}`)}`,
        );
      });

      // Install missing interfaces
      const userConfig = await readUserConfig();
      const interfacesToInstall: {
      ...
      progressBar.start(
        missingInterfaces.length,
        0,
        "Installing missing interfaces",
      );
      ...
      for (const [git, interfaces] of gitGroups) {
        progressBar.update(processedCount, {
          title: `Loading interfaces from ${git}`,
        });
        ...
        for (const interface_ of interfaces) {
          progressBar.update(processedCount, {
            title: `Processing ${interface_.name}@${interface_.version}`,
          });
          ...
          if (interface_.skipInstall) {
            installed.push(
              `${interface_.name}@${interface_.version} (skip-install)`,
            );
            processedCount++;
            continue;
          }

          interfacesToInstall.push({
            interfaceInfo,
        ...
        if (interfacesToInstall.length > 0) {
          progressBar.update(processedCount, {
            title: `Installing interfaces from ${git}`,
          });
      ...
      if (installed.length > 0) {
        success(
          chalk.green`Successfully installed ${installed.length} interface(s):`,
        );
      ...
      if (failed.length > 0) {
        ...
        failed.forEach((item) => {
          info(
            `  ${chalk.red("•")} ${chalk.bold(item.name)} - ${chalk.dim(item.reason)}`,
          );
        });
      }
      ...
      if (totalExisting > 0) {
        info(
          chalk.dim`${totalExisting} interface(s) were already installed and skipped`,
        );
      }
