...
describe("GitDownloader", () => {
  ...
  it("should clone repository and set cache version", async () => {
    ...
    expect(
      execCalls.some((call) => call.command.startsWith("git clone")),
    ).to.equal(true);
  });
  ...
  it("uses the cached copy when git fetch fails", async () => {
    ...
    expect(execCalls.some((call) => call.startsWith("git reset"))).to.equal(
      false,
    );
  });
  ...
  it("re-clones when the manifest has an entry but the repo folder is missing", async () => {
    ...
    expect(result).to.have.length(1);
    expect(execCalls.some((call) => call.startsWith("git clone"))).to.equal(
      true,
    );
    expect(cache.getVersion(cacheKey)).to.equal("git:main:abcdef");
  });
