...
describe("module imports install behavior", () => {
  ...
  it("fails when AntelopeJS config is missing", async () => {
    sinon
      .stub(common, "readModuleManifest")
      .resolves({ name: "test-module" } as any);
  ...
  it("honors skip-install and creates symlinks", async () => {
    ...
    try {
      sinon.stub(common, "readModuleManifest").resolves({
        name: "test-module",
        antelopeJs: {
          imports: [{ name: "foo@1.0.0", skipInstall: true }],
          importsOptional: [],
        },
      } as any);
      sinon
  ...
  it("installs missing interfaces and reports skipped ones", async () => {
    ...
    try {
      ...
      sinon.stub(common, "readModuleManifest").resolves({
        name: "test-module",
        antelopeJs: {
          imports: ["existing@1.0.0", "new@1.0.0"],
          importsOptional: [],
        },
      } as any);
      sinon
