...
describe("Git operations behavior", () => {
  ...
  it("installs interface files and removes them", async () => {
    ...
    try {
      ...
      await fsPromises.writeFile(
        path.join(versionDir, "index.d.ts"),
        "// d.ts",
      );
      ...
      sinon
        .stub(pkgManager, "getInstallPackagesCommand")
        .resolves("echo install");
      ...
      await gitOps.installInterfaces(
  ...
  it("creates @ajs symlinks from interface definitions", async () => {
    ...
    try {
      ...
      await fsPromises.mkdir(path.join(ifaceDir, "nested"), {
        recursive: true,
      });
      await fsPromises.writeFile(path.join(ifaceDir, "1.0.0.d.ts"), "// d.ts");
      await fsPromises.writeFile(
        path.join(ifaceDir, "nested", "2.0.0.d.ts"),
        "// d.ts",
      );
      ...
      expect(existsSync(path.join(ajsBase, "nested", "2.0.0.d.ts"))).to.equal(
        true,
      );
    } finally {
      process.env.HOME = originalHome;
  ...
  it("replaces existing @ajs entries when creating symlinks", async () => {
    ...
    try {
      ...
      await fsPromises.writeFile(
        path.join(ajsBase, "1.0.0.d.ts"),
        "old content",
      );

      await gitOps.createAjsSymlinks(moduleDir);

      const updated = await fsPromises.readFile(
        path.join(ajsBase, "1.0.0.d.ts"),
        "utf8",
      );
  ...
  it("warns when interfaces path is not a directory", async () => {
    ...
    try {
      ...
      await fsPromises.writeFile(
        path.join(ifaceRoot, "interfaces.d"),
        "not a dir",
      );
  ...
  it("throws when checkout fails", async () => {
    ...
    try {
      ...
      execStub
        .onThirdCall()
        .resolves({ code: 1, stdout: "", stderr: "checkout fail" });
  ...
  it("installs git-sourced interface files", async () => {
    ...
    try {
      ...
      await fsPromises.writeFile(
        path.join(interfacePath, "1.0.0.d.ts"),
        "// d.ts",
      );
      ...
      await gitOps.installInterfaces(remote, moduleDir, [
        { interfaceInfo, version: "1.0.0" },
      ]);
  ...
  it("deduplicates interfaces and loads dependency info", async () => {
    ...
    try {
      ...
      await fsPromises.writeFile(
        path.join(depVersionDir, "index.d.ts"),
        "// d.ts",
      );
      ...
      await fsPromises.writeFile(
        path.join(extraVersionDir, "index.d.ts"),
        "// d.ts",
      );
